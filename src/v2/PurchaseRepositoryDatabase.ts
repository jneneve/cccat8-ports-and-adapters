...
export default class PurchaseRepositoryDatabase implements PurchaseRepository {
    ...
    async getPurchases (cardNumber: string, month: number, year: number): Promise<Purchase[]> {
        ...
        const purchasesData = await connection.query("select * from purchase where card_number = $1 and extract(month from date)::integer = $2 and extract(year from date)::integer = $3", [cardNumber, month, year]);
        const purchases = [];
